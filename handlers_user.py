...
async def show_sched_img_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Может быть вызван и из-под текстового сообщения, и из-под фото (кнопка «Назад» после картинки) —
    edit_message_text не работает на сообщении с фото, поэтому в этом случае пересоздаём сообщение."""
    query = update.callback_query
    await query.answer()
